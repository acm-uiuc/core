    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:dynamodb:${var.region}:${data.aws_caller_identity.current.account_id}:table/infra-core-api-store-carts",
...
resource "aws_lambda_function" "api_lambda" {
  ...
  environment {
    variables = {
      ...
      SshIssuerRoleArn                      = aws_iam_role.ssh_issuer_role.arn
...
resource "aws_kms_key" "ssh_ca" {
  description              = "Asymmetric KMS Key for SSH OIDC Certificate Authority"
...
resource "aws_kms_alias" "ssh_ca_alias" {
  name          = "alias/${var.ProjectId}-ssh-oidc-ca"
  target_key_id = aws_kms_key.ssh_ca.key_id
}

resource "aws_iam_role" "ssh_issuer_role" {
  name_prefix = "${var.ProjectId}-ssh-issuer-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "ssh_issuer_policy" {
  name_prefix = "${var.ProjectId}-ssh-issuer-policy-"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "ssh_issuer_attahc" {
  role       = aws_iam_role.ssh_issuer_role.name
  policy_arn = aws_iam_policy.ssh_issuer_policy.arn
}
