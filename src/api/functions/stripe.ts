...
export const createCheckoutSessionWithCustomer = async ({
  ...
  const payload: Stripe.Checkout.SessionCreateParams = {
    ...
    payment_method_types: (delayedSettlementAllowed
      ...
      (x) => captureMethod === "manual" ? x !== "crypto" : true,
...
export const checkOrCreateCustomer = async ({
  ...
  return await lock.using(async () => {
    ...
    const existing = unmarshall(customerResponse.Items![0] as any) as {
    ...
    try {
      ...
      const isTxnCanceled = e?.name === "TransactionCanceledException";
      const hasCondFail =
