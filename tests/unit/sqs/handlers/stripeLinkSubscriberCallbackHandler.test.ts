...
import { deliverSubscriberCallback } from "../../../../src/api/functions/subscriberCallback.js";
...
describe("stripeLinkSubscriberCallbackHandler", () => {
  ...
  test("retries the message when the link is missing", async () => {
    ddbMock.on(QueryCommand).resolvesOnce({ Items: [] });

    await expect(
      stripeLinkSubscriberCallbackHandler(payload, metadata, logger as any),
    ).rejects.toThrow("Stripe link not found for subscriber callback");
  ...
  test("refuses to deliver when the index returns duplicates", async () => {
    ddbMock.on(QueryCommand).resolvesOnce({
      Items: [
        marshall({
          callbackUrl: "https://callbacks.example.com/stripe",
        ...
        marshall({
          callbackUrl: "https://other.example.com/stripe",
          signingSecret: "secret_456",
        }),
      ],
    });
    ...
    await expect(
      stripeLinkSubscriberCallbackHandler(payload, metadata, logger as any),
    ).rejects.toThrow("Multiple Stripe links found for linkId plink_123");

    expect(logger.error).toHaveBeenCalledWith(
      { linkId: "plink_123", itemCount: 2 },
